...
if(isset($_POST['submit']))
{
    ...
    if($ans=="no"){
        $sql = "SELECT no FROM quiz WHERE question=10";
        $result = mysqli_query($conn,$sql);
        $row = mysqli_fetch_array($result);
        $loc = $row['no'];
    }
    else{
        $sql = "SELECT yes FROM quiz WHERE question=10";
        $result = mysqli_query($conn,$sql);
        $row = mysqli_fetch_array($result);
        $loc = $row['yes'];
    }
...
<div class="card">
  <div class="container">
  <center><p>Do you enjoy story telling?</p></center>
<div class="option-group">
  <div class="option-container">
    <input class="option-input" id="option-1" type="radio" name="q10" value="yes"/>
    <input class="option-input" id="option-2" type="radio" name="q10" value="no"/>
