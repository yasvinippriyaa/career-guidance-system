...
if(isset($_POST['submit']))
{
    ...
    if($ans=="no"){
        $sql = "SELECT no FROM quiz WHERE question=12";
        $result = mysqli_query($conn,$sql);
        $row = mysqli_fetch_array($result);
        $loc = $row['no'];
        $_SESSION['result'] = 'therapist';
    }
    else{
        $sql = "SELECT yes FROM quiz WHERE question=12";
        $result = mysqli_query($conn,$sql);
        $row = mysqli_fetch_array($result);
        $loc = $row['yes'];
...
<div class="card">
  <div class="container">
  <center><p>Do you not hesitate to take financial risks?</p></center>
