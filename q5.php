...
<div class="card">
  <div class="container">
  <center><p>Do you love Mathematics and Calculations?</p></center>
<div class="option-group">
  <div class="option-container">

    <input class="option-input" id="option-1" type="radio" name="q5" value="yes"/>
    <input class="option-input" id="option-2" type="radio" name="q5" value="no"/>
...
if(isset($_POST['submit'])){
    ...
    if($ans=="no"){
        ...
        $loc = $row['no'];
        $_SESSION['result'] = 'entrepreneur';
    }
    else{
        $sql = "SELECT yes FROM quiz WHERE question=5";
        $result = mysqli_query($conn,$sql);
        $row = mysqli_fetch_array($result);
        $loc = $row['yes'];
        $_SESSION['result'] = 'analyst';
    }
