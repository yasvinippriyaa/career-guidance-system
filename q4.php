...
<div class="card">
  <div class="container">
  <center><p>Do you want to earn a lot of money while working??</p></center>
<div class="option-group">
  <div class="option-container">

    <input class="option-input" id="option-1" type="radio" name="q4" value="yes"/>
    <input class="option-input" id="option-2" type="radio" name="q4" value="no"/>
...
<div class="btn-group">
  <input name="submit" class="button" type="submit" value="Next">
</div>
</div>
</div>
</form>
...
if(isset($_POST['submit'])){
    ...
    if($ans=="no"){
        $sql = "SELECT no FROM quiz WHERE question=4";
        $result = mysqli_query($conn,$sql);
        $row = mysqli_fetch_array($result);
        $loc = $row['no'];
    }
    else{
        $sql = "SELECT yes FROM quiz WHERE question=4";
        $result = mysqli_query($conn,$sql);
        $row = mysqli_fetch_array($result);
        $loc = $row['yes'];
        $_SESSION['result'] = 'finance';
    }
