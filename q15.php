...
if(isset($_POST['submit']))
{
    ...
    if($ans=="no"){
        $sql = "SELECT no FROM quiz WHERE question=15";
        $result = mysqli_query($conn,$sql);
        $row = mysqli_fetch_array($result);
        $loc = $row['no'];
        $_SESSION['result'] = 'podcaster';
    }
    else{
        $sql = "SELECT yes FROM quiz WHERE question=15";
        $result = mysqli_query($conn,$sql);
        $row = mysqli_fetch_array($result);
        $loc = $row['yes'];
        $_SESSION['result'] = 'digitalmarketing';
    }
    mysqli_close($conn); header($loc);    
}
...
<div class="card">
  <div class="container">
  <center><p>Do you like spending time on social media? Would you like to get paid for it?</p></center>
<div class="option-group">
  <div class="option-container">
    <input class="option-input" id="option-1" type="radio" name="q15" value="yes"/>
    <input class="option-input" id="option-2" type="radio" name="q15" value="no"/>
