...
if(isset($_POST['submit']))
{
    ...
    if($ans=="no"){
        $sql = "SELECT no FROM quiz WHERE question=11";
        $result = mysqli_query($conn,$sql);
        $row = mysqli_fetch_array($result);
        $loc = $row['no'];
        $_SESSION['result'] = 'production';
    }
    else{
        $sql = "SELECT yes FROM quiz WHERE question=11";
        $result = mysqli_query($conn,$sql);
        $row = mysqli_fetch_array($result);
        $loc = $row['yes'];
        $_SESSION['result'] = 'cs';
...
<div class="card">
  <div class="container">
  <center><p>Do you love to work with computers?</p></center>
