...
if(isset($_POST['submit']))
{
    ...
    if($ans=="no"){
        $sql = "SELECT no FROM quiz WHERE question=14";
        $result = mysqli_query($conn,$sql);
        $row = mysqli_fetch_array($result);
        $loc = $row['no'];
    }
    else{
        $sql = "SELECT yes FROM quiz WHERE question=14";
        $result = mysqli_query($conn,$sql);
        $row = mysqli_fetch_array($result);
        $loc = $row['yes'];
        $_SESSION['result'] = 'writer';
    }
...
<div class="card">
  <div class="container">
  <center><p>Are you well versed in grammar?</p></center>
<div class="option-group">
  <div class="option-container">
    <input class="option-input" id="option-1" type="radio" name="q14" value="yes"/>
    <input class="option-input" id="option-2" type="radio" name="q14" value="no"/>
